...
<!-- END SIDEBAR -->
    ...
    <div class="span12">
        ...
        <!-- END PAGE TITLE & BREADCRU/Application/Admin/View/-->
            ...
            <!--内容区域-->
            <div id="main">
                <div style="text-align: right"><a class="btn" href="<?php echo U('contact/add');?>">关于我们添加</a></div>
                <table class="table table-striped table-bordered table-hover mytable">
                    <tr style="height: 50px;"><th>ID</th><th>公司名称</th><th>二维码</th><th>全国热线</th><th>电话</th><th>地址</th><th>邮编</th><th>传真</th><th>邮箱</th><th>操作</th></tr>
                    <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><span class="label label-info"><?php echo ($vo['cid']); ?></span></td>
                            <td><span class="label label-success"><?php echo ($vo['title']); ?></span></td>
                            <td><img src="/<?php echo ($vo['erweima']); ?>" width="50px" height="50px"></td>
                            <td><span class="label label-important"><?php echo ($vo['hot']); ?></span></td>
                            <td><span class="label label-warning"><?php echo ($vo['tel']); ?></span></td>
                            <td><span class="label label-success"><?php echo truncate_zh($vo['address']);?></span></td>
                            <td><span class="label label-inverse"><?php echo ($vo['youbian']); ?></span></td>
                            <td><span class="label label-success"><?php echo ($vo['fax']); ?></span></td>
                            <td><span class="label label-important"><?php echo ($vo['email']); ?></span></td>
                            <td><i class="icon-pencil"></i> <a href="<?php echo U('contact/update','cid='.$vo['cid']);?>">编辑</a> | <i class="icon-trash"></i> <a href="<?php echo U('contact/delete','cid='.$vo['cid']);?>" onclick="return confirm('亲，数据无价，真的忍心删除吗？')" >删除</a></td>
