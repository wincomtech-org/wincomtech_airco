...
<!-- END SIDEBAR -->
    ...
    <div class="span12">
        ...
        <!-- END PAGE TITLE & BREADCRU/Application/Admin/View/-->
            ...
            <!--内容区域-->
            <div id="main">
                <div style="text-align: right"><a class="btn" href="<?php echo U('server/add');?>">服务和支持添加</a></div>
                <table class="table table-striped table-bordered table-hover mytable">
                    <tr style="height: 50px;"><th>ID</th><th>内容</th><th>操作</th></tr>
                    <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><span class="label label-info"><?php echo ($vo['sid']); ?></span></td>
                            <td><span class="label label-success"><?php echo truncate_zh(strip_tags($vo['contents']),50,"...");?></span></td>
                            <td><i class="icon-tag"></i> <a href="<?php echo U('server/views','sid='.$vo['sid']);?>" target="_blank">查看</a> | <i class="icon-pencil"></i> <a href="<?php echo U('server/update','sid='.$vo['sid']);?>">编辑</a> | <i class="icon-trash"></i> <a href="<?php echo U('server/delete','sid='.$vo['sid']);?>" onclick="return confirm('亲，数据无价，真的忍心删除吗？')" >删除</a></td>
