...
<!-- END SIDEBAR -->
    ...
    <div class="span12">
        ...
        <!-- END PAGE TITLE & BREADCRU/Application/Admin/View/-->
            ...
            <!--内容区域-->
            <div id="main">
                <div style="text-align: right"><a class="btn" href="<?php echo U('journey/add');?>">添加发展历程</a></div>
                <table class="table table-striped table-bordered table-hover mytable">
                    <tr style="height: 50px;"><th>ID</th><th>发展历程</th><th>操作</th></tr>
                    <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><span class="label label-info"><?php echo ($vo['jid']); ?></span></td>
                            <td><span class="label label-success"><?php echo truncate_zh($vo['contents'],50,"...");?></span></td>
                            <td><i class="icon-pencil"></i> <a href="<?php echo U('journey/update','jid='.$vo['jid']);?>">编辑</a> | <i class="icon-trash"></i> <a href="<?php echo U('journey/delete','jid='.$vo['jid']);?>" onclick="return confirm('亲，数据无价，真的忍心删除吗？')" >删除</a></td>
