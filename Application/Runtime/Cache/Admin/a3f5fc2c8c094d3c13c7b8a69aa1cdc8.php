...
<!-- END SIDEBAR -->
    ...
    <div class="span12">
        ...
        <!-- END PAGE TITLE & BREADCRU/Application/Admin/View/-->
            ...
            <!--内容区域-->
            <div id="main">
                <div style="text-align: right"><a class="btn" href="<?php echo U('blogs/add');?>">新闻添加</a></div>
                <table class="table table-striped table-bordered table-hover mytable">
                    <tr style="height: 50px;"><th>新闻ID</th><th>新闻图片</th><th>新闻标题</th><th>发布时间</th><th>操作</th></tr>
                    <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><span class="label label-info"><?php echo ($vo['bid']); ?></span></td>
                            <td><img src="/<?php echo ($vo['blogimage']); ?>" width="50px" height="50px"></td>
                            <td><span class="label label-success"><?php echo truncate_zh(strip_tags($vo['title']),50,"...");?></span></td>
                            <td><span class="label label-inverse"><?php echo (date("Y-m-d H:i:s",$vo['createtime'])); ?></span></td>
                            <td><i class="icon-tag"></i> <a href="<?php echo U('blogs/views','bid='.$vo['bid']);?>" target="_blank">查看</a> | <i class="icon-pencil"></i> <a href="<?php echo U('blogs/update','bid='.$vo['bid']);?>">编辑</a> | <i class="icon-trash"></i> <a href="<?php echo U('blogs/delete','bid='.$vo['bid']);?>" onclick="return confirm('亲，数据无价，真的忍心删除吗？')" >删除</a></td>
