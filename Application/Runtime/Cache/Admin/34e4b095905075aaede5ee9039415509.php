...
<!-- END SIDEBAR -->
    ...
    <div class="span12">
        ...
        <!-- END PAGE TITLE & BREADCRU/Application/Admin/View/-->
            ...
            <!--内容区域-->
            <div id="main">
                <div style="text-align: right"><a class="btn" href="<?php echo U('product/add');?>">产品添加</a></div>
                <table class="table table-striped table-bordered table-hover mytable">
                    <tr style="height: 50px;"><th>产品ID</th><th>产品图片</th><th>关键字</th><th>证书编号</th><th>产品名称</th><th>型号</th><th>空调类型</th><th>冷暖类型</th><th>功率</th><th>适用面积</th><th>工作方式</th><th>操作</th></tr>
                    <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><span class="label label-info"><?php echo ($vo['pid']); ?></span></td>
                            <td><img src="/<?php echo ($vo['productimage']); ?>" width="50px" height="50px"></td>
                            <td><span class="label label-success"><?php echo truncate_zh($vo['keywords'],20,"...");?></span></td>
                            <td><span class="label label-inverse"><?php echo ($vo['certificate']); ?></span></td>
                            <td><span class="label label-important"><?php echo truncate_zh($vo['title'],20,"...");?></span></td>
                            <td><span class="label label-warning"><?php echo ($vo['num']); ?></span></td>
                            <td><span class="label label-success"><?php echo ($vo['ktype']); ?></span></td>
                            <td><span class="label label-important"><?php echo ($vo['wtype']); ?></span></td>
                            <td><span class="label label-warning"><?php echo ($vo['power']); ?></span></td>
                            <td><span class="label label-inverse"><?php echo ($vo['area']); ?></span></td>
                            <td><span class="label label-success"><?php echo ($vo['func']); ?></span></td>
                            <td><i class="icon-tag"></i> <a href="<?php echo U('product/views','pid='.$vo['pid']);?>" target="_blank">查看</a> | <i class="icon-pencil"></i> <a href="<?php echo U('product/update','pid='.$vo['pid']);?>">编辑</a> | <i class="icon-trash"></i> <a href="<?php echo U('product/delete','pid='.$vo['pid']);?>" onclick="return confirm('亲，数据无价，真的忍心删除吗？')" >删除</a></td>
